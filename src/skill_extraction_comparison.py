...
# logger.info(f"ANTHROPIC_API_KEY: {os.environ.get('ANTHROPIC_API_KEY')}")
...
class SkillExtractorBERT:
    ...
    def __init__(
        ...
        Args:
            model_name: HuggingFace model name for base ModernBERT (default: answerdotai/ModernBERT-base)
            embed_model: HuggingFace model for embeddings (default: nomic-ai/modernbert-embed-base)
        ...
        # Load base ModernBERT model for any custom processing
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.model = AutoModel.from_pretrained(model_name).to(device)
        self.threshold = threshold

        # Use ModernBERT embeddings for semantic similarity
        try:
            self.sentence_transformer = SentenceTransformer(embed_model)
            logger.info(f"Successfully loaded {embed_model} with SentenceTransformer")
        except Exception as e:
            logger.warning(
                f"Failed to load {embed_model} with SentenceTransformer: {e}"
            )
            logger.info("Falling back to all-MiniLM-L6-v2 for sentence embeddings")
            self.sentence_transformer = SentenceTransformer("all-MiniLM-L6-v2")

    def _extract_embedding(self, text):
        """Extract embeddings from text using the ModernBERT embedding model"""
        # Use sentence_transformer for consistent embeddings
        return self.sentence_transformer.encode([text], convert_to_numpy=True)

    def extract_skills(
        self, text: str, all_skills: List[Dict[str, Any]], top_k=10
    ) -> List[Dict[str, str]]:
        """
        ...
        for skill in all_skills:
            ...
            if text_embedding.shape[1] != skill_emb.shape[0]:
                logger.warning(
                    f"Embedding dimension mismatch: {text_embedding.shape} vs {skill_emb.shape}"
                )
                continue

            # Cosine similarity
            similarity = np.dot(text_embedding, skill_emb) / (
                np.linalg.norm(text_embedding) * np.linalg.norm(skill_emb)
            )
...
class SkillExtractorLLM:
    ...
    def extract_skills(
        self, text: str, all_skills: List[Dict[str, Any]]
    ) -> List[Dict[str, str]]:
        """
...
"""
    ...
    def _map_to_esco(
        self, extracted_skills: List[Dict[str, str]], all_skills: List[Dict[str, Any]]
    ) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if "sentence_embedding" in all_skills[0]:
                all_skill_embeddings = np.array(
                    [skill["sentence_embedding"] for skill in all_skills]
                )
            else:
            ...
            for skill_obj in extracted_skills:
                ...
                # Convert arrays to same dtype (float32) to avoid dtype mismatch
                skill_embedding = np.array(skill_embedding, dtype=np.float32)
                all_skill_embeddings = np.array(all_skill_embeddings, dtype=np.float32)
            ...
            for skill_obj in extracted_skills:
                ...
                for skill in all_skills:
                    ...
                    score = SequenceMatcher(
                        None, skill_name.lower(), skill["skill_name"].lower()
                    ).ratio()
                    if (
                        score > best_score and score > 0.7
                    ):  # 0.7 threshold for decent match
                        best_score = score
                        best_match = skill
...
def evaluate_extractor(
    ...
    for item in tqdm(subset, desc=f"Evaluating {data_type}"):
        ...
        if data_type == "course":
            ...
            text = (
                f"{item['text']['personalInformation']['name']}. Skills: {skills_text}"
            )
        ...
        y_pred = [
            1 if skill in extracted_skill_names else 0 for skill in all_skill_names
        ]
...
def run_comparison(models=["all"], data_limit=100):
    ...
    all_skills = load_esco_skills(
        "src/synthetic_data_generator/skills_en.csv",
    ...
    if include_models["bert"]:
        ...
        for data_type, data in [
            ...
            metrics = evaluate_extractor(
                bert_extractor, data, all_skills, data_type, data_limit
            )
    ...
    for model_type in llm_types:
        for shot_setting in ["zero", "few"]:
            ...
            try:
                llm_extractor = SkillExtractorLLM(
                    model_type=model_type, shot_setting=shot_setting
                )
                ...
                for data_type, data in [
                    ...
                    metrics = evaluate_extractor(
                        llm_extractor, data, all_skills, data_type, data_limit
                    )
...
def plot_results(results_df):
    ...
    results_df["model_shot"] = results_df.apply(
        lambda row: (
            f"{row['model']}"
            if row["shot_setting"] == "N/A"
            else f"{row['model']} ({row['shot_setting']}-shot)"
        ),
        axis=1,
    )
    ...
    shot_comparison = (
        llm_results.groupby(["model", "shot_setting"])[
    ...
    logger.info(
        "Zero-shot vs Few-shot comparison saved to zero_vs_few_shot_comparison.png"
    )
...
def load_esco_skills(
    ...
    Args:
        ...
        embed_model: Name of the embedding model to use for semantic similarity
    ...
    logger.info(
        f"Computing embeddings for skills using models compatible with {bert_model_name}"
    )
    ...
    try:
        ...
        # BERT Transformer for the BERT-based model
        try:
            bert_transformer = SentenceTransformer(embed_model)
            logger.info(f"Successfully loaded {embed_model} for ModernBERT embeddings")
        except Exception as e:
            logger.warning(
                f"Failed to load {embed_model} with SentenceTransformer: {e}"
            )
            logger.info("Falling back to base model for embeddings")
            bert_transformer = SentenceTransformer(bert_model_name)

        # Standard sentence transformer for LLM-based skill matching
        ...
        for i in range(0, len(skills), batch_size):
            ...
            # Compute embeddings with ModernBERT embed model
            bert_embeddings = bert_transformer.encode(
        ...
        for skill, bert_emb, sent_emb in zip(
            skills, all_bert_embeddings, all_sentence_embeddings
        ):
