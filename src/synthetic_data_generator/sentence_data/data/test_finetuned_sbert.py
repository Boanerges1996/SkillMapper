...
logging.info("Starting model loading and testing script for fine-tuned SBERT...")

# 1. Load the fine-tuned SentenceTransformer model.
# Replace "finetuned_sentence_transformer" with the path where you saved your model.
model = SentenceTransformer("finetuned_sentence_transformer")
logging.info("Fine-tuned SentenceTransformer model loaded successfully.")


# 2. Load your JSON data to extract the unique skills.
...
# Extract a unique list of skills.
unique_skills = list({item["preferredLabel"] for item in all_data})
logging.info(f"Number of unique skills: {len(unique_skills)}")

# 3. Precompute embeddings for all unique skills.
# This step converts all skill names into embeddings in one batch.
...
# 4. Define a function to predict the top matching skills for a given input text.
def predict_skills(input_text, top_k=5):
    # Compute the embedding for the input text.
    input_embedding = model.encode(input_text, convert_to_tensor=True)

    # Compute cosine similarity scores between the input embedding and all skill embeddings.
    cosine_scores = util.cos_sim(input_embedding, skill_embeddings)[0]

    # Retrieve the top-k skills with the highest similarity scores.
    top_results = torch.topk(cosine_scores, k=top_k)

    # Pair each skill with its corresponding similarity score.
    predicted = [
        (unique_skills[idx], score.item())
        for idx, score in zip(top_results[1], top_results[0])
    ]
    return predicted


# 5. Test the model with a sample course description.
course_example = """
...
"""
predicted_skills = predict_skills(course_example, top_k=10)
